...
// в разделе настройка:другие настройки - настраивается количество попыток запуска двигателя.

// в разделе настройка:настройка функций - настраивается функция задержки работы стартера.
...
// РАСШИРЕНАЯ НАСТРОЙКА (не трогать!)
...
unsigned long resMill;
unsigned long thisMill;

void blink(int bCount = 1){ //функция блинк
...
inline void starterChekExt(){  // функция задержки стартера расширеная

	chk_GENA = digitalRead(IN_GENA_PIN);
	lastMill = millis();
	resMill = 0;

	while(chk_GENA == 0 && resMill < STARTER_TIMEOUT){
		chk_GENA = digitalRead(IN_GENA_PIN);
		thisMill = millis();
		resMill = thisMill - lastMill;
	}
}

inline void starterChek(){  // функция задержки стартера
	delay(STARTER_TIMEOUT);
}

void genStart(){  // функция запуска генератора
  ...
  Serial.println("podsos on");
	...
	while(starterCount < ZAPUSK_COUNTER_VAL - 1){
		...
		digitalWrite(LED_PIN, LOW);
		if(SET_STARTER_TIMER_FUNC > 0){
			Serial.println("starter timeout mode 1");
			Serial.print(resMill / 1000);
	    	Serial.print(" s    ");
	    	Serial.print(resMill);
	    	Serial.println(" ms");
		}else{
			Serial.println("starter timeout mode 0");
			Serial.print("delay(");
			Serial.print(STARTER_TIMEOUT);
			Serial.println(");");
		}
...
void genStop(){  // функция остановки генератора
...
void setup() {  // точка входа. установки
...
void loop() {  // цикл. мониторинг сети
